	...
	parseSwiperData = (value) => {
		...
		if (p.hasTrack1) {
			if (p.account_name != null)
				result['name'] = p.account_name;
			else
				result['name'] = p.surname + ' ' + p.firstname;						
		...
		if (strParse != '') {
			...
			if (bTrack1_2) {
				...
				if (arrayStrSwipe.length > 2) {
					this.account = this.stripAlpha(arrayStrSwipe[0].substring(1, arrayStrSwipe[0].length));
			...
			if (bTrack1) {
				...
				if (arrayStrSwipe.length > 2) {
					this.account = sAccountNumber = this.stripAlpha(arrayStrSwipe[0].substring(1, arrayStrSwipe[0].length));
